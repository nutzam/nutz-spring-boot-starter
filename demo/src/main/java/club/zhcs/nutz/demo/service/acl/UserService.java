...
import club.zhcs.nutz.demo.config.AdministratorConfigurationProperties;
...
import club.zhcs.nutz.demo.vo.InstalledAction;
import club.zhcs.nutz.demo.vo.InstalledModule;
...
public class UserService extends IdNameBaseService<User> {
    ...
    public void init() {
        ...
        if (suUser == null) {
            suUser = save(User.builder()
                              ...
                              .pwd(PasswordUtils.encode(configurationProperties.getCypher(), su))
                              .build());
        }
        // 角色
    ...
    public boolean grant(long id, List<String> actionInfos) {
        userPermissionService.clear(Cnd.where(USER_ID, "=", id));
    ...
    public boolean grantRole(long id, List<Long> roles) {
        userRoleService.clear(Cnd.where(USER_ID, "=", id));
