 ...
 */
@RestController
@Schema(name = "Action", description = "功能动作模块")
public class ActionController {
    ...
    public Result<Action> get(@Parameter(name = "功能动作id") @PathVariable("id") long id) {
        return Result.success(actionService.fetch(id));
    }

    @PostMapping("action")
    @Operation(summary = "新增功能动作")
    public Result<Action> add(@Parameter(name = "功能动作数据") @RequestBody Action action) {
        return Result.success(actionService.save(action));
    }

    @PutMapping("action")
    @Operation(summary = "根据id更新功能动作")
    public Result<Void> edit(@Parameter(name = "功能动作数据") @RequestBody Action action) {
        return actionService.update(action, "name", "key") ? Result.success() : Result.fail("更新操作失败");
    }

    @DeleteMapping("action/{id}")
    @Operation(summary = "根据id获取功能动作详情")
    public Result<Void> delete(@Parameter(name = "功能动作id") @PathVariable("id") long id) {
